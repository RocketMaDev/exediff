...
#define NOTE_DELETE(ctxt, xoff) \
    note_byte_delete((xoff))

#define NOTE_INSERT(ctxt, yoff) \
    note_byte_insert((yoff))
                ...
                const uint8_t *data2, size_t pos2, size_t limit2)
{
    ...
    if (max_compare >= FAST_SKIP_THRESHOLD) {
        ...
        while (snake_length + block_size <= max_compare) {
            if (memcmp(data1 + pos1 + snake_length, 
                      data2 + pos2 + snake_length, 
                      block_size) == 0) {
    ...
    if (max_compare >= FAST_SKIP_THRESHOLD) {
        ...
        while (snake_length + block_size <= max_compare) {
            if (memcmp(data1 + pos1 - snake_length - block_size, 
                      data2 + pos2 - snake_length - block_size, 
                      block_size) == 0) {
                snake_length += block_size;
            } else {
                break;
...
static bool
enhanced_compareseq(ptrdiff_t xoff, ptrdiff_t xlim, 
...
static double
quick_similarity_check(const struct file_data *file1, const struct file_data *file2)
{
...
static int
compare_files_optimized(const char *file1, const char *file2, 
                       bool verbose, bool minimal)
{
    struct context ctx;
    int result = 0;
