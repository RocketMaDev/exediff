...
#define FAST_SKIP_THRESHOLD                                                   \
  1024 /* Use fast skipping for blocks larger than this */
...
static size_t
find_snake_fast (const uint8_t *data1, size_t pos1, size_t limit1,
...
static size_t
find_snake_reverse_fast (const uint8_t *data1, size_t pos1, size_t start1,
      ...
      while (snake_length + block_size <= max_compare)
        {
          if (memcmp (data1 + pos1 - snake_length - block_size,
                      data2 + pos2 - snake_length - block_size, block_size)
              == 0)
            {
              snake_length += block_size;
            }
...
static bool
enhanced_compareseq (ptrdiff_t xoff, ptrdiff_t xlim, ptrdiff_t yoff,
                     ptrdiff_t ylim, bool find_minimal, struct context *ctxt)
{
  ...
  size_t back_snake
      = find_snake_reverse_fast (data1, xlim, xoff, data2, ylim, yoff);
  if (back_snake > 0)
    {
  ...
  if (current_diff.verbose)
    {
      printf ("DELETE: byte at offset %td (0x%02x) from file 1\n", offset,
              current_diff.files[0].data[offset]);
    }
  ...
  if (current_diff.verbose)
    {
      printf ("INSERT: byte at offset %td (0x%02x) from file 2\n", offset,
              current_diff.files[1].data[offset]);
    }
  current_diff.change_count++;
...
static int
read_file (const char *filename, struct file_data *file)
{
  FILE *fp;
  struct stat st;
...
static void
free_file (struct file_data *file)
{
  if (file->data)
    {
      free (file->data);
...
static double
quick_similarity_check (const struct file_data *file1,
...
static int
compare_files_optimized (const char *file1, const char *file2, bool verbose,
                         bool minimal)
{
  struct context ctx;
  int result = 0;
  ...
  printf ("Comparing '%s' (%zu bytes) with '%s' (%zu bytes)\n", file1,
          current_diff.files[0].size, file2, current_diff.files[1].size);
  ...
  ptrdiff_t diags = (ptrdiff_t)(current_diff.files[0].size
                                + current_diff.files[1].size + 3);
  ...
  printf ("Using threshold: %td, heuristic: %s\n", ctx.too_expensive,
          ctx.heuristic ? "enabled" : "disabled");
  ...
  size_t total_bytes
      = MAX (current_diff.files[0].size, current_diff.files[1].size);
  if (total_bytes > 0)
    {
      double skip_percentage
          = (double)current_diff.total_snake_length * 100.0 / total_bytes;
  ...
  if (current_diff.change_count == 0)
  ...
  else
    {
      ...
      if (total_bytes > 0)
        {
          double diff_percent
              = (double)current_diff.change_count * 100.0 / total_bytes;
...
print_usage (const char *program_name)
{
  ...
  printf ("  -m, --minimal    Find minimal differences (slower but more "
          "accurate)\n");
...
main (int argc, char *argv[])
{
  ...
  for (unsigned i = 0; i < deleted->size; i++)
    printf ("DELETED: %u, %ld [%02x]\tINSERTED: %u, %ld [%02x]\n", i,
            deleted->array[i], current_diff.files[0].data[deleted->array[i]],
            i, inserted->array[i],
            current_diff.files[1].data[inserted->array[i]]);

  mmap_file old_file
      = { current_diff.files[0].data, current_diff.files[0].size };
  mmap_file new_file
      = { current_diff.files[1].data, current_diff.files[1].size };
